...
struct SettingsView: View {
    ...
    var body: some View {
        NavigationSplitView {
            List(SettingsSection.allCases, selection: Binding(
                ...
                HStack {
                    ...
                    if item == .providers,
                       settingsStore.provider.requiresAPIKey,
        ...
        .onChange(of: settingsStore.model) { _, _ in
            languageSearch = ""
    ...
    private var providers: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(alignment: .top, spacing: 12) {
                ...
                Spacer()
                if settingsStore.provider.requiresAPIKey {
            ...
            SettingsCard(L10n.t("prov.connection")) {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 12) {
                    ...
                    if selectedModelInfo.languageCodes.isEmpty {
                        Label(
                            L10n.tf("prov.languagesCount", selectedModelInfo.languageCountText),
                            systemImage: "globe"
                        )
                        .foregroundStyle(.secondary)
                    } else {
                ...
                if !selectedModelInfo.languageCodes.isEmpty,
                   let code = settingsStore.language.apiCode,
                ...
                Divider()
                if settingsStore.provider == .local {
                    Button(L10n.t("local.manage")) {
                        appState.selectedSettingsSection = SettingsSection.localModels.rawValue
                    }
                } else {
    ...
    private var localModelsView: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text(L10n.t("local.privacy"))
                .foregroundStyle(.secondary)
            Text(L10n.t("local.memoryPolicy"))
                .foregroundStyle(.secondary)

            if settingsStore.provider == .local {
                HStack {
                    Label(
                        L10n.tf("local.selectedProvider", settingsStore.model),
                        systemImage: "checkmark.circle.fill"
                    )
                    .foregroundStyle(.green)
                    Spacer()
                    Button(L10n.t("local.chooseCloud")) {
                        appState.selectedSettingsSection = SettingsSection.providers.rawValue
                    }
                }
            }

            ForEach(LocalSpeechModel.allCases) { model in
                SettingsCard(model.rawValue) {
                    HStack(alignment: .top) {
                        VStack(alignment: .leading, spacing: 5) {
                            Text("\(model.approximateSize) · \(model.detail)")
                                .foregroundStyle(.secondary)
                            localModelStatus(model)
                        }
                        Spacer()
                        localModelActions(model)
                    }
                }
            }
        }
    ...
    @ViewBuilder
    private func localModelStatus(_ model: LocalSpeechModel) -> some View {
        switch localModels.state(for: model) {
        case .notInstalled:
            Text(L10n.t("local.notInstalled"))
                .font(.footnote)
                .foregroundStyle(.secondary)
        case let .downloading(_, status):
            HStack {
                ProgressView().controlSize(.small)
                Text(status)
            }
        case .installed:
            if localModels.loadingModel == model {
                Label(L10n.t("local.loading"), systemImage: "hourglass")
                    .font(.footnote)
                    .foregroundStyle(.orange)
            } else if localModels.loadedModel == model {
                Label(L10n.t("local.inMemory"), systemImage: "memorychip.fill")
                    .font(.footnote)
                    .foregroundStyle(.green)
            } else if isSelected(model) {
                Label(L10n.t("local.selectedLazy"), systemImage: "checkmark.circle")
                    .font(.footnote)
                    .foregroundStyle(.blue)
            } else {
                Label(L10n.t("local.downloadedUnloaded"), systemImage: "internaldrive")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
    ...
    @ViewBuilder
    private func localModelActions(_ model: LocalSpeechModel) -> some View {
        switch localModels.state(for: model) {
        case .installed:
            HStack {
                if localModels.loadedModel == model {
                    Button(L10n.t("local.unload")) {
                        localModels.unload()
                    }
                } else if isSelected(model) {
                    Button(L10n.t("local.selected")) {}
                        .disabled(true)
                } else {
                    Button(L10n.t("local.use")) {
                        settingsStore.provider = .local
                        settingsStore.model = model.rawValue
                    }
                }
                Button(L10n.t("local.remove"), role: .destructive) {
        ...
        case .notInstalled, .failed:
            Button(L10n.t("local.download")) {
                localModels.download(model)
            }
        }
    }

    private func isSelected(_ model: LocalSpeechModel) -> Bool {
        settingsStore.provider == .local && settingsStore.model == model.rawValue
    }
    ...
    private var updates: some View {
        SettingsCard {
            Toggle(L10n.t("upd.checkOnLaunch"), isOn: $settingsStore.automaticallyCheckUpdates)
                .onChange(of: settingsStore.automaticallyCheckUpdates) { _, value in
                    updateManager.configure(automaticallyChecks: value)
                }
