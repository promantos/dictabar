...
enum ProviderConnectionTester {
    /// ~0.35s mono 16 kHz 16-bit LE silence (matches Dictabar recorder format).
    static func run(
        settings: ProviderSettings,
        apiKey: String,
        audioURL: URL? = nil
    ) async throws -> String {
        guard !settings.provider.requiresAPIKey
    ...
    private static func writeSilentWAV(durationSeconds: Double) throws -> URL {
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("Dictabar-test-\(UUID().uuidString)")
