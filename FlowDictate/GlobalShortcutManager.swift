...
final class GlobalShortcutManager {
    ...
    func start(preset: ShortcutPreset, customShortcut: KeyboardShortcut) {
        ...
        if let keyCode = preset.keyCode {
            let ok = startModifierTap(keyCode: CGKeyCode(keyCode))
            if !ok {
                // Fallback so dictation still works without Input Monitoring.
                DiagnosticsLogger.shared.log("shortcut: modifier tap failed — falling back to Carbon \(customShortcut.display)")
                onError("Right-modifier shortcut needs Input Monitoring. Using \(customShortcut.display) until then. Menu bar Start/Stop always works.")
                startCarbon(shortcut: customShortcut.keyCode == 0 ? .defaultDictation : customShortcut)
            }
        } else {
    ...
    private func startModifierTap(keyCode: CGKeyCode) -> Bool {
        ...
        // Prefer defaultTap so macOS requires Input Monitoring and lists the app.
        // Fall back to listenOnly if defaultTap is unavailable but listen works.
        ...
        guard let eventTap else {
            // Failed defaultTap is what registers the app in Input Monitoring.
            DiagnosticsLogger.shared.log("shortcut: CGEvent.tapCreate failed — registering IM list entry")
